function [predictedY,predictedY_ST,W,W_max]...
    = stacking_predictions_METAFUNCTION(Yin,Din,QpredictedY_store,QYin_store,sigma_store,alph_store,options,metafeatures_store,varargin)
...
allcs = [];
if (nargin>8) && ~isempty(varargin{1}) % if correlation structure is defined (it is defined in nargin = 4 and then we check it's not empty)
...
n_metafeatures =  size(metafeatures_store,2)/5;
...
%         QYin_store{ifold} = QYin;
        ...
        Qmetafeature = metafeatures_store(ji,:);
        
        QA = NaN(size(ji,2),n_metafeatures*M);
        for i = 1:n_metafeatures
              QA(:,M*i-M+1:M*i) = Qmetafeature(:,M*i-M+1:M*i).*QpredictedY;
        end
...
%         size(QpredictedY)
        ...
        % repetitions of HMM
        %w = lsqlin(QpredictedY,QYin,[],[],ones(1,M),1,zeros(M,1),ones(M,1),[],opts1); %w = lsqlin(QpredictedY,QYin,[],[],[],[],zeros(M,1),ones(M,1),[],opts1);
        w_FWLS = lsqlin(QA,QYin,[],[],ones(1,M*n_metafeatures),1,zeros(M*n_metafeatures,1),ones(M*n_metafeatures,1),[],opts1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        metafeature_ST = metafeatures_store(J,:);
        A_ST = NaN(size(J,2),n_metafeatures*M);
         
%         size(metafeature_ST);
%         size(A_ST);
%         size(predictedY_ST(J,:));

        for i = 1:n_metafeatures
              A_ST(:,M*i-M+1:M*i) = metafeature_ST(:,M*i-M+1:M*i).*predictedY_ST(J,:);
        end
