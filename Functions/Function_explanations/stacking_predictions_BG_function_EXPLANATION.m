function [predictedY,predictedY_ST,W,W_max]...
    = stacking_predictions_BG(Yin,Din,QpredictedY_store,QYin_store,sigma_store,alph_store,options,varargin)
...
allcs = [];
if (nargin>5) && ~isempty(varargin{1}) % if correlation structure is defined (it is defined in nargin = 4 and then we check it's not empty)
