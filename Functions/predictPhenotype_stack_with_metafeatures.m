function [predictedY,predictedY_FWLS,W_ridge,W_FWLS,predictedY_ST]...
    = predictPhenotype_stack_with_metafeatures(Yin,Din,metafeatures,options,varargin)
%
...
% Set up parameters for CV ridge regression parameter optimisation (uses
% netspredict5)
parameters = struct();
...
%         end
        ...
        %%%%%%%%% RIDGE REGRESSION %%%%%%%%%%%%%
        % uncenter data for ridge regression (otherwise we could add the intercept back in)
        QpredictedY_uncenter = QpredictedY + my;
        QYin_uncenter = QYin + my;
        
        % inner loop for ridge regression regulatrization parameter
        %[stats,~,~,~,~,~,lambda_best] = nets_predict5_BG(QYin,QpredictedY,'gaussian',parameters);
        [stats,~,~,~,~,~,lambda_best] = nets_predict5_BG(QYin,QpredictedY,'gaussian',parameters);
        lambda = lambda_best;
        %lambda = mode(betas)
        
        % Determine stacking weights (using best regularization parameter)
         w_ridge = (QpredictedY_uncenter' * QpredictedY_uncenter + lambda * eye(M)) \ (QpredictedY_uncenter' * QYin_uncenter);
...
%         w = lsqlin(QpredictedY_uncenter,QYin_uncenter,[],[],ones(1,M),1,zeros(M,1),ones(M,1),[],opts1);      
        ...
        for i = 1:n_metafeatures
              A_train(:,M*i-M+1:M*i) = meta_train(:,M*i-M+1:M*i).*QpredictedY_uncenter;
        end
        ...
        %w_FWLS = lsqlin(A_train,QYin_uncenter,[],[],[ones(1,M) zeros(1,M*(n_metafeatures-1))],1,zeros(M*n_metafeatures,1),ones(M*n_metafeatures,1),[],opts1);
        w_FWLS = (A_train' * A_train + lambda * eye(M*n_metafeatures)) \ (A_train' * QYin_uncenter);
        W_FWLS(:,ifold) = w_FWLS;
        %%%%%%%%% METAFEATURES END %%%%%%%
        ...
        predictedY(J,ii) = predictedY_ST(J,:,ii) * w_ridge;
...
%         mean_prediction = mean(predictedY(J,ii))
%         mean_target = mean(Yin(J,:))
%         predictedY(J,ii) = predictedY(J,ii) + (mean_target - mean_prediction);