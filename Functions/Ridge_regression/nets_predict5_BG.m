function [stats,predictedY,predictedY0,predictedYD,predictedYD0,beta,lambda_best] ...
    = nets_predict5_BG(Yin,Xin,family,parameters,varargin)
...
% we want to specify method as ridge
...
%%% can only use Gaussian family for ridge
...
% if regularization parameters have not been set then use these
...
% i.e. here we check if the method is either lasso or glmnet
% and sets enet = 1 if the method is lasso or glmnet
...
% tmpnm = tempname returns a unique string, tmpnm, suitable for use as a temporary filename.
tmpnm = tempname; 
% if strcmpi(Method,'glmnet')
%     mkdir(tmpnm); mkdir(strcat(tmpnm,'/out')); mkdir(strcat(tmpnm,'/params'));
% end

%%% Can't use multinomial for ridge regression so ignore
...
Xin = Xin - repmat(mx,N,1); % centre Xin by subtracing mean from each column of Xin
Xin(:,sx>0) = Xin(:,sx>0) ./ repmat(sx(sx>0),N,1); % divide by standard deviation
...
% set permutations empty unless given. Nperm is set to 1 if not given
...
% else
    ...
    predictedYpD = zeros(N,1); predictedYpD0 = zeros(N,1); % deconfounded variables
% end
...
%     end
    ...
    if isempty(CVfolds)
        if CVscheme(1)==1
            ...
            folds = cvfolds_np(Yin,family,CVscheme(1),allcs);
        end
    else
        folds = CVfolds;
    end
    ...
    Dev_all = Inf(length(folds),length(alpha));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Outer CV loop %%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         end
        ...
        % create the inner CV structure - stratified for family=multinomial
        Qfolds = cvfolds_np(Y,family,CVscheme(2),Qallcs);

        % initialise deviance variable
        Dev = Inf(nlambda,length(alpha));
        Lambda = {};
        
        %%% if the method is not unregularized, then follow this
        if ~strcmpi(Method,'unregularized')
            
            %%% for each regularization parameter (in the case of ridge
            %%% regression)
            for ialph = 1:length(alpha)
                
%                 if strcmpi(family,'multinomial') && strcmpi(Method,'glmnet')
%                     QpredictedYp = Inf(QN,q,nlambda);
%                     % q = number of features so we need this if multinomial
%                 else
...
%                 end
                ...
                % Inner CV loop - to determine best alpha (which is lambda
                % in case of ridge regression
                for Qifold = 1:length(Qfolds)
                    QJ = Qfolds{Qifold}; % test data
                    Qji = setdiff(1:QN,QJ); % training data
                    QX = QXin(Qji,:); %training values of predictors
                    QY = QYin(Qji,:); % training values of responses
%                     if enet % if lasso or glmnet
...
%                             estimation.a0 = lassostats.Intercept;
                        ...
                        %%% Ridge regression method (family must be
                        %%% Gaussian)
                        case {'ridge','Ridge'}
                            if strcmpi(family,'gaussian')
                                estimation = struct();
                                ...
                                estimation.beta = (QX' * QX + R) \ (QX' * QY); % determine weights
%                             else
%                                 error('Only Gaussian family is implemented for ridge method');
                            end
                    end
                    
%%%%%%% enet = 1 if method is glmnet or lasso (so ignore since we want ridge)
%                     if Qifold == 1 && enet
%                         Lambda{ialph} = estimation.lambda;
%                         options = rmfield(options,'nlambda');
%                         %rmfield removes 'nlambda' from the structure option
%                     end
...
%                     Qdev = -2 * Qdev;
            ...
            Dev_all(ifold,:) = Dev;
            % note the index of the parameter that minimizes the deviance 
            [~,opt] = min(Dev(:));
...
%                 end
%               
            %%% if the method is ridge regression, then we use the best 
            %%% alpha to make the prediction?
            else % ridge
                ialph = opt; % note the index of the best alpha
                options.alpha = alpha(ialph); % set alpha to best the best regularization parameter
                if verbose, fprintf('Alpha chosen to be  %f \n',options.alpha); end
                estimation = struct(); estimation0 = struct();
                X = [ones(size(X,1),1) X]; 
                R =  options.alpha * ridg_pen_scale * eye(size(X,2)); R(1,1) = 0;
                estimation.beta = (X' * X + R) \ (X' * Y); % determine weights
        ...
        %%% if the method is unregularized, then follow this
        else
%             estimation = struct(); estimation0 = struct();
%             if strcmpi(family,'gaussian')
%                 X = [ones(size(X,1),1) X]; 
...
%         end
        ...
        if perm==1 && ~strcmpi(Method,'unregularized')
            ...
            % find best parameter
             ...
             [~, min_idx] = min(sum(Dev_all));
             lambda_best = alpha(min_idx);
