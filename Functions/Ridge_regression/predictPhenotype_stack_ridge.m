...
%         end
        ...
        % uncenter data for ridge regression (otherwise we could add the
        % intercept back in)
        QpredictedY_uncenter = QpredictedY + my;
        QYin_uncenter = QYin + my;
        
        % determine best ridge regression regulatrization parameter
        [stats,~,~,~,~,~,lambda_best] = nets_predict5_BG(QYin_uncenter,QpredictedY_uncenter,'gaussian',parameters);
        %[stats,~,~,~,~] = nets_predict5(QYin,QpredictedY,'gaussian',parameters); lambda_common = mode(stats.alpha)   
        lambda = lambda_best;
        
        % Determine stacking weights (using best parameter)
         w = (QpredictedY_uncenter' * QpredictedY_uncenter + lambda * eye(M)) \ (QpredictedY_uncenter' * QYin_uncenter);
         W(:,ifold) = w;     
