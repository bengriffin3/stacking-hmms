function [predictedY,predictedY_ST, predictedYD,YD,stats,...
    predictedY_max,predictedYD_max,stats_max,W,W_max]...
    = predictPhenotype_stack_random_forest(Yin,Din,options,varargin)
%
...
%         end
        ...
        for Qifold = 1:length(Qfolds)
            QJ = Qfolds{Qifold}; Qji = setdiff(1:QN,QJ);
            QY = QYin(Qji,:); Qmy = 0;%mean(QY); QY = QY-Qmy;
            Nji = length(Qji);
            for m = 1:M
                QD = QDin{m}(Qji,Qji);
                QD2 = QDin{m}(QJ,Qji);
...
%         W(:,ifold) = w;
        ...
        Mdl_rf = fitrensemble(QpredictedY,QYin,'Method','Bag');
...
%         predictedY(J,ii) = predictedY_ST(J,:,ii) * w;
        ...
        predictedY(J,ii) = predict(Mdl_rf,predictedY_ST(J,:,ii));
