...
if stochastic_learn
    ...
    % Gamma entropy&LL
    fe = fe + sum(evalfreeenergy_single_subject([],Tmat,Gamma,Xi,hmm,[],[],[1 0 1 0 1]));
    ...
    for i = 1:length(T)
        ...
        if ~isempty(Xi)
            [f,l] = evalfreeenergy_single_subject(X,Ti,Gamma(t,:),Xi(t2,:,:),hmm,residuals,XX,[0 1 0 0 0]);
            % f = the variational free energy
            % l = log likelihood of the observed data, per trial
            fell = fell + sum(f); %  data likelihood
            ll = [l; ll];
        else
            [f,l] = evalfreeenergy_single_subject(X,Ti,Gamma(t,:),[],hmm,residuals,XX,[0 1 0 0 0]);
            fell = fell + sum(f); %  data likelihood
            ll = [l; ll];
        end
    end
    ...
    [fe,ll] = evalfreeenergy_single_subject(data,T,Gamma,Xi,hmm,residuals);
    fe = sum(fe);
end
