function [predictedY_ridge,W_ridge,predictedY_ST]...
    = predictPhenotype_stack_ridge_V2(Yin,Din,options,varargin)
%
...
predictedY_ridge = zeros(N,q); predictedY_max = zeros(N,q); predictedY_ST = zeros(N,M,q);
...
%%%%%%%%%%%%%%%% Stacking w/ ridge regression %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        [stats,~,~,~,~] = nets_predict5(QYin,QpredictedY,'gaussian',parameters);
        lambda = mode(stats.alpha);
        
        % Determine stacking weights (using best regularization parameter)
         w_ridge = (QpredictedY' * QpredictedY + lambda * eye(M)) \ (QpredictedY' * QYin);
         %W_ridge(:,ifold) = w_ridge       
       
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            % Make the predictions for the 5 HMM repetitions
            for m = 1:M
