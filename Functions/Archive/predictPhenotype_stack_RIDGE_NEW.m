function [predictedY,predictedYD,YD,stats,...
    ...
    = predictPhenotype_stack_RIDGE_NEW(Yin,Din,options,varargin)
%
...
predictedY_lambda = zeros(N,q,length(lambda));
...
%         end
        ...
        explained_variance_lambda = NaN(length(lambda),1);
        for ilambda = 1:length(lambda)
            lamb = lambda(ilambda);

            w = (QpredictedY' * QpredictedY + lamb * eye(M)) \ (QpredictedY' * QYin);
            %w = ridge(QYin,QpredictedY,lamb,0);

            %opts1 = optimset('display','off');
            %w = lsqlin(QpredictedY,QYin,[],[],ones(1,M),1,zeros(M,1),ones(M,1),[],opts1);
            %w = lsqlin(QpredictedY,QYin,[],[],[],[],[],[],[],opts1);
            W(:,ifold) = w;
...
%         W_max(best,ifold) = 1;
        ...
        predictedY_lambda(J,ii,ilambda) = predictedY_ST(J,:,ii) * w;
        
        %predictedY(J,ii) = predictedY_ST(J,:,ii) * w(2:end) + w(1);
        % should probably change this to error rather than explained
        % variance
        explained_variance_lambda(ilambda) = corr(squeeze(predictedY_lambda(J,ii,ilambda)),Yin(J,:)).^2;
        end
        
        [mx,best_lambda] = max(explained_variance_lambda);
        predictedY(J,ii) = predictedY_lambda(J,ii,best_lambda);
