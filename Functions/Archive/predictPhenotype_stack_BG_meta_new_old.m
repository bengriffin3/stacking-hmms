function [predictedY,predictedY_ST,W_FWLS,W_max,QpredictedY_store,QYin_store,folds,sigma_store,alph_store]...
    = predictPhenotype_stack_BG_meta_new(Yin,Din,metafeatures, options,varargin)
...
if q == 1
    ...
    if any(which_nan)
        Yin = Yin(~which_nan);
        metafeatures = metafeatures(~which_nan,:);
...
QpredictedY_store = cell(length(folds),1);
QYin_store = cell(length(folds),1);
sigma_store = cell(length(folds),1);
alph_store = cell(length(folds),1);
...
%         % Now we have chosen the best alpha's and sigma's!
        ...
        QpredictedY_store{ifold} = QpredictedY;
        QYin_store{ifold} = QYin;
        sigma_store{ifold} = sigma;
        alph_store{ifold} = alph;
        
        
        QA = NaN(size(ji,2),n_metafeatures*M);
        for i = 1:n_metafeatures
              QA(:,M*i-M+1:M*i) = meta_train(:,M*i-M+1:M*i).*QpredictedY;
        end
        ...
        % repetitions of HMM
          w = lsqlin_BG(QpredictedY,QYin,[],[],ones(1,M),1,zeros(M,1),ones(M,1),[],opts1);
        %w = quadprog(QpredictedY,QYin,[],[],ones(1,M*n_metafeatures),1,zeros(M*n_metafeatures,1),ones(M*n_metafeatures,1),[],opts1);
          %w = lsqlin_BG(QpredictedY,QYin,[],[],ones(1,M),1,[],[],[],opts1);
        W(:,ifold) = w;
       

%         w_FWLS = lsqlin_BG(QA,QYin,[],[],ones(1,M*n_metafeatures),1,zeros(M*n_metafeatures,1),ones(M*n_metafeatures,1),[],opts1);
% % 
% %         %w_FWLS = quadprog(QA,QYin,[],[],ones(1,M*n_metafeatures),1,zeros(M*n_metafeatures,1),ones(M*n_metafeatures,1),[],opts1);
% %         %w_FWLS = lsqlin_BG(QA,QYin,[],[],ones(1,M*n_metafeatures),1,[],[],[],opts1);
%         W_FWLS(:,ifold) = w_FWLS;
%         w
%         w_FWLS
...
% 
%             A_ST = NaN(size(J,2),n_metafeatures*M);
%         for i = 1:n_metafeatures
%             A_ST(:,M*i-M+1:M*i) = meta_test(:,M*i-M+1:M*i).*predictedY_ST(J,:);
%         end



        predictedY(J,ii) = predictedY_ST(J,:,ii) * w; % MULTIPLY THE TEST PREDICTIONS WITH THE BEST WEIGHTS ACROSS THE TRAINING DATA
%         predictedY(J,ii) = A_ST * w_FWLS;
