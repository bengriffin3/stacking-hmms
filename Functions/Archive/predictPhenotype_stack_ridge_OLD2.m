function [predictedY_ridge,W_ridge,predictedY_ST]...
    = predictPhenotype_stack_ridge(Yin,Din,options,varargin)
%
...
lambda = [0.00001 0.0001 0.001 0.01 0.1 0.4 0.7 0.9 1.0 2.5 5.0 10 100];
...
predictedY_lambda = zeros(N,length(lambda),q); predictedY_max = zeros(N,q); predictedY_ST = zeros(N,M,q);
...
W_ridge = zeros(M,length(lambda),length(folds));
...
%%%%%%%%%%%%%%% inner loop for stacking estimation END %%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%% Stacking w/ ridge regression %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for ilambda = 1:length(lambda)
            ...
            w_ridge_cv = (QpredictedY' * QpredictedY + lamb * eye(M)) \ (QpredictedY' * QYin);
            %w_ridge_cv = ridge(QYin,QpredictedY,lamb);
            W_ridge(:,ilambda,ifold) = w_ridge_cv;
            ...
            for m = 1:M
                ...
                predictedY_lambda(J,ilambda,ii) = predictedY_ST(J,:,:) * w_ridge_cv;
        end

    end
        
        % Now we have the best lambda's (and the corresponsing weights), we
        % just want to predict the test fold and combine
...
explained_variance_ridge = corr(squeeze(predictedY_lambda),Yin).^2;
[M,I] = max(explained_variance_ridge);
lambda_best = lambda(I);
predictedY_ridge = predictedY_lambda(:,I,:);
