...
f = data;
n_sessions= 4
T_subject{1} = repmat(1200,n_sessions,1)';
...
repetitions = 1%5; % to run it multiple times (keeping all the results)
...
explained_variance = NaN(size(vars,2),1);
% vars_hat = NaN(N,size(vars,2)); % the predicted variables
vars_hat = NaN(N,size(vars,2)); % need to -2 for removed subjects?

vars(:,28) = zeros(1003,1);
for k = 1:length(intelligence_variable_vec)% we do only the intelligence variables % 1:size(vars,2)
    j = intelligence_variable_vec(k);
...
save('staticFC_predictions_intelligence.mat','vars_hat','explained_variance')
...
save('HMM_predictions_intelligence.mat','vars_hat','explained_variance')
