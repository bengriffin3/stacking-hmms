...
DirSame = ['FC_HMM_zeromean_1_covtype_full_8_states/']; % set directory for same states repetitions
DirVarying = ['FC_HMM_zeromean_1_covtype_full_vary_states_3_14/']; % set directory for varying states repetitions
...
vars_hat_hmm_varying_states = vars_hat(subKEEP,:,:);
...
DirMultiRep = ['FC_HMM_zeromean_1_covtype_full_vary_states_63_reps/']; % set directory for multiple reps of varying states repetitions
...
error_vec_plot = NaN(15,1);
mean_ev_across_vars = mean(explained_variance(:,1:45));
for i = 1:15
