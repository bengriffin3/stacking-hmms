...
% let's run a test for 20 subjects
%grotKEEP(21:end) = 0;
...
Y = Y_all;
vars = Y;
headers_vars = headers_grouped_category(to_use_all,:);
...
category_index_vec(nan_variables) = [];
[B,idx] = sort(category_index_vec);
% reorg_vector = a_test_reorg(idx)

%% Examine just ingelligence variables
ind1 = strcmp(headers_grouped_category(:,3), 'Intelligence');
A1 = headers_grouped_category(ind1,:);


ind2 = strcmp(headers_grouped_category(:,3), 'Intelligence');
A2 = headers_grouped_category(ind1,:);

idxKEEP = true(99,1);
idxKEEP(strcmp(A2(:,2), 'Working Memory Task')) = 0; % Working memory task all vars
idxKEEP(strcmp(A2(:,2), 'Language Task')) = 0; % Language task all vars
idxKEEP(strcmp(A2(:,2), 'Relational Task')) = 0; % Relational memory task all vars
idxKEEP(32) = true; % add actual task vars
idxKEEP(40) = true;
idxKEEP(46) = true;
