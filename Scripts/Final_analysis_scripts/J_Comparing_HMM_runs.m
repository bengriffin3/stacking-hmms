...
%DirOut = 'C:\Users\au699373\OneDrive - Aarhus Universitet\Dokumenter\MATLAB\HMMMAR_BG\HMMMAR Results\MAR_2022\Investigating_bad_predictions\Repetition_6_SC\';
...
n_subjects = size(target_vars.vars,1);
n_vars = size(target_vars.vars,2);

% load prediction 1
%HMM_predictions1 = load([DirOut 'HMM_predictions_same_folds.mat'], 'vars_hat','subject_squared_error');
HMM_predictions1 = load([DirOut 'HMM_predictions_distmatgauss.mat'], 'vars_hat','subject_squared_error','explained_variance'); % load predictions

% load prediction 2
HMM_predictions2 = load([DirOut 'HMM_predictions_rep_1.mat'], 'vars_hat','subject_squared_error','explained_variance');
...
%HMM_predictions = load([DirOut 'HMM_predictions_distmatgauss.mat'], 'vars_hat','subject_squared_error'); % load predictions
HMM_predictions = load([DirOut 'HMM_predictions.mat'], 'vars_hat','subject_squared_error'); % load predictions


figure;
for j = 1:n_vars%[1 5 10 15] % pick intelligence feature
    var_target = target_vars.vars(:,j);
    for i = 1:25%size(HMM_predictions.vars_hat,3)
...
HMM_predictions = load([DirOut 'HMM_predictions_LOO_hyper_tune.mat'], 'vars_hat','subject_squared_error');
...
for j = 11%[1 5 10 15] % pick intelligence feature
    ...
    for i = 1:size(HMM_predictions.vars_hat,3)
        ...
        % Targets + errors
        figure(803); subplot(5,5,count); scatter(var_target,mse_plot1,'x'); hold on; xlabel(sprintf('Y (var %i)',j)); ylabel('Error');
        sgtitle(sprintf('Error for variable %i with max = %.4g, mean = %.4g, min = %.4g',j,max(var_target), mean(var_target),min(var_target)));
        ...
        % plot values outside of range in red
        if sum(nnz_idx_vh) ~= 0
            figure(803); scatter(var_target(nnz_idx_vh),mse_plot1(nnz_idx_vh),'x','r');
        end
...
for j = 1
    ...
    % same scale
    %norm_sse = normc(squeeze(HMM_predictions.subject_squared_error(:,j,:)));
    A = squeeze(HMM_predictions.subject_squared_error(:,j,:));
    norm_sse = A./sum(A);
    ...
    clim([0 0.01]);
    xlabel('HMM repetition'); ylabel('Subject No.')
end

%% Table of terrible predictions
clc
%bad_subject_predictions_high = zeros(n_subjects,1);
%bad_subject_predictions_low = zeros(n_subjects,1);

bad_subject_predictions_high_var = zeros(n_subjects,n_vars);
bad_subject_predictions_low_var = zeros(n_subjects,n_vars);

for j = 1:n_vars  % pick intelligence feature
    var_target = target_vars.vars(:,j);
    bad_subject_predictions_high = zeros(n_subjects,1);
    bad_subject_predictions_low = zeros(n_subjects,1);
...
bad_subject_predictions_high_var_new= [(1:n_subjects)' bad_subject_predictions_high_var_new]; % add idx of subjects
