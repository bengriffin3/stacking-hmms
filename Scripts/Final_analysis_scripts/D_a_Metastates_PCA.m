...
for i = 1:rep
    ...
    subplot(5,3,i); imagesc(C(ord,ord)); % figure 2 (b)
    title(sprintf('FO Correlation Matrix - HMM with K = %i',K)); xlabel('States');
...
FO_metastate2_new = [ -FO_metastate2(:,1) FO_metastate2(:,[2 3 4]), -FO_metastate2(:,5:end)];
disp(corr(FO_metastate1_new)); % Check signs have been sorted correctly (all elements of this matrix should be close to 1)
