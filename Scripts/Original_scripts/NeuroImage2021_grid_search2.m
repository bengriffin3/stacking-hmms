...
K_vec = repelem([3 5 7 9 11 13 15],3)';
...
%         for n1 = 1:N
%             for n2 = n1+1:N
%                     DistHMM{r}(n1,n2,d) = (hmm_kl(out.HMMs_dualregr{n1},out.HMMs_dualregr{n2}) ...
%                     + hmm_kl(out.HMMs_dualregr{n2},out.HMMs_dualregr{n1}))/2;
% 
%                % 
%                 %DistHMM(n2,n1,r,d) = DistHMM(n1,n2,r,d);
...
% bar(X,Y)
% xlabel('Number of states'), ylabel('Total explained vaiance')
% legend('DD 10','DD 100','DD 1000','DD 10000')
% title('Total explained variance across 34 variables for changing dirichlet diag')
...
% e=zeros(34,1);
% for i=1:34
%     vars_hat_reshape = reshape(vars_hat(:,i,:,:),[1001  28]);
...
% yyaxis right
% scatter(1:34,mean(reshape(explained_variance,[34 28]),2),'k','x');
% [B,I] = sort(e);
...
%     exp_var = reshape(explained_variance,[34 28]);
%     Y = exp_var(i,:);
%     
