...
zeromean = 1;
repetitions = 5;%repetitions = 63; % to run it multiple times (keeping all the results)
...
for r = 1:repetitions
    ...
    for j = 4%1:size(vars,2)
        ...
        [yhat,stats] = predictPhenotype_LOO(y_new,D_new_HMM,parameters_prediction,twins_new,conf_new);
...
save('HMM_predictions_LOO_V4.mat','vars_hat','explained_variance')
...
n_subjects = 100;
Entropy_subject = NaN(n_subjects,5);
likelihood_subject = NaN(n_subjects,5);
K_vec = [3 5 8 11 14];
for i = 1:5
    i
    K = K_vec(i)
...
load([ DirOut 'HMMs_meta_data_subject.mat'])
...
%     metafeatures(:,6:end) = ((metafeatures(:,6:end) - mean(metafeatures(:,6:end)))./std(metafeatures(:,6:end)))+1
    ...
    for j = 1:size(vars,2)
        ...
        % Stacked predictions
        [yhat_ST,yhat_stack,yhat_stack_static,yhat_FWLS,yhat_FWLS_norm,yhat_ridge,w_stack,w_stack_static,w_FWLS,w_FWLS_norm,w_ridge] = predictPhenotype_stack_meta_V2(y_new,D_new_rep_HMM,D_new_static,metafeatures_new,parameters_prediction,twins_new,conf_new);
    ...
    legend('FWLS predictor','FWLS normalized predictor','Ridge predictor','Stacked predictor','Stacked with static predictor','Static predictor','3 States','5 States','8 States','11 States','14 States')
    title(sprintf('Metafeature %i',mf))
end

%% Predictions of behaviour using the HMMs (STACKED RIDGE)
rng('default') % set seed for reproducibility
build_regression_data_V2_0;
vars0 = vars;
%%% LOAD FOR PREDICTION
...
DirOut = ['/Users/bengriffin/OneDrive - Aarhus Universitet/Dokumenter/MATLAB/HMMMAR_BG/HMMMAR Results/FC_HMM_zeromean_1_covtype_full_stack_vs_63_reps/Best_5_reps/']; % Test folder

load([ DirOut  'KLdistances_ICA50_BEST_5_reps'])
load([ DirOut 'HMMs_meta_data_subject_BEST_5_reps'])

MF1 = ones(size(vars,1),repetitions); % first metafeature is the constant one
MF2 = Entropy_subject;
MF3 = likelihood_subject;
% MF4 = K_all_reps; % Max possible K for repetitions = (3,5,8,11,14)
% MF5 = maxFO_all_reps;
% MF6 = switchingRate_all_reps;
normalized_MF2 = (MF2-min(MF2))./(max(MF2)-min(MF2));
normalized_MF2_mean_1 = normalized_MF2 - mean(normalized_MF2) + 1;
normalized_MF3 = (MF3-min(MF3))./(max(MF3)-min(MF3));
normalized_MF3_mean_1 = normalized_MF3 - mean(normalized_MF3) + 1;
%standardized_MF2 = ((MF2 - mean(MF2)) ./ std(MF2)) + 1;
%standardized_MF3 = ((MF3 - mean(MF3)) ./ std(MF3)) + 1;

%metafeatures_all = {MF1, log10(MF2), MF3, MF4, log(MF5), log10(log10(MF6))};
metafeatures_all = {MF1, normalized_MF2_mean_1, normalized_MF3_mean_1};

metafeatures = [metafeatures_all{1} metafeatures_all{2} metafeatures_all{3}];% metafeatures_all{3}];
n_metafeatures = size(metafeatures,2)/repetitions;

DistHMM = DistMatKEEP(:,:,1:3);
repetitions = size(DistHMM,3);
% Initialise variables (stacking)
W_stack = NaN(repetitions,10,1,size(vars,2)); % a weight for every repetition of the HMM, for every fold, for every intelligence feature
W_ridge = NaN(repetitions,10,size(vars,2)); % a weight for every repetition of the HMM, for every fold, for every intelligence feature
...
explained_variance_stack = NaN(size(vars,2),1);
explained_variance_ridge = NaN(size(vars,2),1);
explained_variance_stack_FWLS = NaN(size(vars,2),1);

prediction_error_ST = NaN(size(vars,2),repetitions);
prediction_error_stack = NaN(size(vars,2),1);
prediction_error_ridge = NaN(size(vars,2),1);
prediction_error_stack_FWLS = NaN(size(vars,2),1);

vars_hat_ST = NaN(size(vars,1),size(vars,2),repetitions); % each repetition of HMM prediction
vars_hat_stack = NaN(size(vars,1),size(vars,2),1); % stacked prediction
vars_hat_FWLS = NaN(size(vars,1),size(vars,2),repetitions); % metafeatures prediction

%vars = (vars-min(vars))./(max(vars)-min(vars)); % Rescale variables 0-1
...
for j = 1:size(vars,2)
    ...
    %[yhat_stack_ridge,~,~,~,~,~,~,W,~,yhat_ST] = predictPhenotype_stack_ridge(y_new,D_new_rep_HMM,parameters_prediction,twins_new,conf_new);
     [yhat_stack_ridge,yhat_stack_FWLS,W_ridge,W_FWLS,yhat_ST] = predictPhenotype_stack_with_metafeatures(y_new,D_new_rep_HMM,metafeatures,parameters_prediction,twins_new,conf_new);
    ...
    % Calculate explained variance
    explained_variance_ST(j,:) = corr(squeeze(yhat_ST),y_new).^2;
    explained_variance_ridge(j) = corr(squeeze(yhat_stack_ridge),y_new).^2;
    explained_variance_stack_FWLS(j) = corr(squeeze(yhat_stack_FWLS),y_new).^2;
    
    % Calculate prediction errors 
    % should I divide this by n? I reckon so - it's then mean prediction
    % errors
    prediction_error_ST(j,:) = sum((yhat_ST - y_new).^2);
    prediction_error_ridge(j) = sum((yhat_stack_ridge - y_new).^2);
    prediction_error_stack_FWLS(j) = sum((yhat_stack_FWLS - y_new).^2);
    ...
    vars_hat_stack_store = NaN(size(vars,1),1);
    vars_hat_stack_store(non_nan_idx) = yhat_stack_ridge;
    vars_hat_stack(:,j) = vars_hat_stack_store;
    
    vars_hat_FWLS_store = NaN(size(vars,1),1);
    vars_hat_FWLS_store(non_nan_idx) = yhat_stack_FWLS;
    vars_hat_FWLS(:,j) = vars_hat_FWLS_store;
...
X = 1:size(vars,2);
figure
scatter(X,explained_variance_ridge,'o','m') % plot my stacked value
hold on
scatter(X,explained_variance_stack_FWLS,'o','b') % plot my stacked value with metafeature
for m = 1:repetitions
    %scatter(X,explained_variance_ST(:,m),'x','MarkerEdgeColor',colour_repetitions{m}/255,'MarkerFaceColor',colour_repetitions{m}/255);%colour_repetitions{m}) % plot the original 5 repetitions of HMM
    scatter(X,explained_variance_ST(:,m),'x')
end
...
xlabel('Intelligence features'); ylabel('r^2')
legend('Ridge Regression', 'Metafeatures','3 states','5 states','8 states','11 states','14 states')


% Plot prediction errors
X = 1:size(vars,2);
figure
scatter(X,prediction_error_ridge/1001,'o','m') % plot my stacked value
hold on
scatter(X,prediction_error_stack_FWLS/1001,'o','b') % plot my stacked value with metafeature
for m = 1:repetitions
    scatter(X,prediction_error_ST(:,m)/1001,'x')
end
% Format chart
title('Prediction errors')
xlabel('Intelligence features'); ylabel('r^2')
legend('Ridge Regression', 'Metafeatures')
