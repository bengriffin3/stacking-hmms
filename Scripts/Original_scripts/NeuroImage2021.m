...
%K_vec = 12:30;
hmm_train_dirichelt_diag_vec = [10 100 1000 10000];
%hmm_train_dirichelt_diag_vec = [10 100 1000 10000 100000];
...
vars_hat_stack = NaN(N,size(vars,2)); % the predicted variables

explained_variance_ST = NaN(size(vars,2),repetitions);
vars_hat_ST = NaN(N,size(vars,2),repetitions); % the predicted variables
...
%         D_new_2 = cell(repetitions,1);
%         for i = 1:repetitions; D_new_2{i} = D_new; end
%         [yhat,yhat_ST] = predictPhenotype_stack_random_forest(y_new,D_new_2,parameters_prediction,twins_new,conf_new);
% 
%         explained_variance_stack(j) = corr(squeeze(yhat),y_new).^2;
%         vars_hat_store = NaN(size(vars,1),1);
%         vars_hat_store(non_nan_idx) = yhat;
%         vars_hat_stack(:,j) = vars_hat_store;
% 
%         explained_variance_ST(j,:) = corr(squeeze(yhat_ST),y_new).^2;
%         vars_hat_store = NaN(size(vars,1),repetitions);
%         vars_hat_store(non_nan_idx,:) = yhat_ST;
%         vars_hat_ST(:,j,:) = vars_hat_store;
...
%save([DirOut 'HMM_predictions_RF.mat'],'vars_hat','explained_variance')

%% Predictions of behaviour using the HMMs (KNN) 
rng('default')
...
explained_variance_KNN = NaN(size(vars,2),repetitions,dirichlet_test);
vars_hat_KNN = NaN(N,size(vars,2),repetitions,dirichlet_test); % the predicted variables
...
for r = 1:repetitions
    ...
    for d = 1:dirichlet_test
        ...
        for j = 1:size(vars,2)
            ...
            parameters_prediction.method = 'NN';
            [yhat,stats,~,~,beta] = predictPhenotype_NN(y_new,D_new,parameters_prediction,twins_new,conf_new);
            explained_variance_KNN(j,r,d) = corr(squeeze(yhat),y_new).^2;
            vars_hat_store = NaN(size(vars,1),1);
            vars_hat_store(non_nan_idx) = yhat;
            vars_hat_KNN(:,j,r,d) = vars_hat_store;
...
%% Predictions of behaviour using the HMMs (FO)

explained_variance_FO = NaN(size(vars,2),repetitions,dirichlet_test);
vars_hat_FO = NaN(N,size(vars,2),repetitions,dirichlet_test); % the predicted variables
...
for r = 1:repetitions
    ...
    for d = 1:dirichlet_test
    %load([DirOut 'HMMs_r' num2str(r) '_states_' num2str(K) '.mat'],'FOdual');  
    load([DirOut 'HMMs_r' num2str(r) '_d' num2str(d) '_states_' num2str(K) '.mat'],'FOdual');
    ...
    for j = 1:size(vars,2)
        ...
        if any(which_nan)
            y_new = y(~which_nan);
            FOdual_new = FOdual(~which_nan,:);
            conf_new = conf(~which_nan,:);
            twins_new = twins(~which_nan,~which_nan);
            warning('NaN found on Yin, will remove...')
        end

        %%% PREDICTIONS USING FO MATRICES

        [yhat] = predictPhenotype_FO(y_new,FOdual_new,parameters_prediction,twins_new,conf_new);
       

        explained_variance_FO(j,r,d) = corr(squeeze(yhat),y_new).^2;
        vars_hat_store = NaN(size(vars,1),1);
        vars_hat_store(non_nan_idx) = yhat;
        vars_hat_FO(:,j,r,d) = vars_hat_store;
...
save([DirOut 'HMM_predictions_FO.mat'],'vars_hat_FO','explained_variance_FO')
...
FO_metastate1 = NaN(N,repetitions);
FO_metastate2 = NaN(N,repetitions);
FO_PCs = NaN(N,14,repetitions);
...
for i = 1:repetitions
    ...
    FO_PCs(:,1:K-1,i) = coeff; % need to sort out signs of these? + make sure saving the rg
...
metastate_profile = FO_metastate_new_1 - FO_metastate_new_2;
 ...
 save([DirOut 'HMMs_meta_data_GROUP_NEW.mat'],...
    'maxFO_all_reps','K_all_reps','switchingRate_all_reps','FO_metastate_new_1','FO_metastate_new_2','metastate_profile','FO_PCs_aligned')
