...
datadir = '/Users/bengriffin/Library/CloudStorage/OneDrive-AarhusUniversitet/Dokumenter/MATLAB/HMMMAR_BG/Behavioural Variables/';
vars = readmatrix([ datadir 'vars.txt']);

p = size(vars,2);

twins = readmatrix([ datadir 'twins.txt']);
twins = twins(2:end,2:end);
load headers_with_category.mat
grotKEEP = true(size(vars,1),1);
grotKEEP(vars(:,1)==376247)=0; % remove subjects that PALM can't cope with
grotKEEP(vars(:,1)==168240)=0; % remove subjects that PALM can't cope with
